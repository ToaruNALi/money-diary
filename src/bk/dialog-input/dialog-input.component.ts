...
import { DialogInputUsecase } from 'src/app/bk/dialog-input/dialog-input.usecase';
      ...
      this.form.get(data.id)?.valueChanges.subscribe((value) => {
        if (data.type === 'date') {
          ...
          if (!!value && typeof value !== 'string') {
            this.form
              .get(data.id)
              ?.setValue(cvtDateToStr(new Date(value)), { emitEvent: false });
          }
        }
        callSetter();
      });
